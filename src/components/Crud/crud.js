import { initData, download, newInitData } from "@/api/data";
import { parseTime, downloadFile } from "@/utils/index";
import { collectionTaskApi } from "@/api/collction/taskApi";
import Vue from "vue";
import qs from "qs";
...
function CRUD(options) {
  const defaultOptions = {
    ...
    newAddRefresh: false,
    //采集
    newAddCollRefresh: false,
    //政治图片审核策略
    strategyNewAddStatus: false
  };
  ...
  const data = {
    ...
    status: {
      ...
      application: CRUD.STATUS.APPLIC,
      // 添加或编辑状态
      get cu() {
        if (
      ...
      get applc() {
        if (this.application === CRUD.STATUS.APPLIC) {
          return CRUD.STATUS.APPLIC;
        }
        throw new Error("wrong crud's application status");
      }
    },
    msg: {
      submit: "提交成功",
  ...
  const methods = {
    ...
    toQuery() {
      crud.page.page = 1;
      if (crud.newAddRefresh) {
        crud.newRefresh();
      } else if (crud.newAddCollRefresh) {
        crud.getCollectionTask();
      } else {
        crud.refresh();
      }
    },
      ...
      return new Promise((resolve, reject) => {
        ...
        initData(crud.url, crud.getQueryParams())
          .then(data => {
            ...
            //crud.page.total = data.totalElements?data.totalElements:data.data.total;
            crud.page.total = data.totalElements?data.totalElements:data.data? data.data.total : data.content.length;
            crud.data = data.content?data.content:data.data ? data.data.list : data.content;
    ...
    newRefresh() {
      ...
      return new Promise((resolve, reject) => {
        ...
        document.getElementById('loading').style.display='block';
        // 请求数据
        newInitData(crud.url, crud.getQueryParams())
          .then(data => {
            const table = crud.getTable();
            if (table && table.lazy) {
              // 懒加载子节点数据，清掉已加载的数据
          ...
          .catch(err => {
            //crud.loading = false;
            document.getElementById('loading').style.display='none';
            crud.notify("接口请求异常", CRUD.NOTIFICATION_TYPE.ERROR);
            reject(err);
          });
      });
    },
    ...
    getCollectionTask() {
      ...
      return new Promise((resolve, reject) => {
        ...
        document.getElementById('loading').style.display='block';
        // 请求数据
        collectionTaskApi(crud.url, crud.getCollectionQueryParams())
          .then(data => {
            const table = crud.getTable();
            if (table && table.lazy) {
              // 懒加载子节点数据，清掉已加载的数据
    ...
    submitCU(mgs) {
      ...
      crud.findVM("form").$refs["form"].validate(valid => {
        ...
        if (!callVmHook(crud, CRUD.HOOK.afterValidateCU, mgs)) {
          return;
        }
        if (crud.form.newAddStatus) {
          crud.doAdd();
    ...
    doAdd() {
      ...
      if (crud.newAddRefresh) {
        if (crud.form.domains) {
          if (!crud.form.blackDomains) {
            crud.form.blackDomains = "";
          }
        }
      }
      crud.crudMethod
        ...
        .then(res => {
          ...
          if (crud.strategyNewAddStatus) {
              if(res.code == 200){
                ...
                crud.status.add = 1;
                crud.notify(res.message, CRUD.NOTIFICATION_TYPE.ERROR);
              }
          }else{
            crud.resetForm();
            if (crud.newAddRefresh) {
    ...
    wzExport() {
      crud.downloadLoading = true;
      download("sensitiveDetail/exportDetailData", crud.getQueryParams())
        .then(result => {
          downloadFile(result, "错敏信息", "xlsx");
          crud.downloadLoading = false;
        })
        .catch(() => {
          crud.downloadLoading = false;
    ...
    getQueryParams: function() {
      // 清除参数无值的情况
      if (!crud.newAddRefresh || this.query.newAddAuthority) {
        Object.keys(crud.query).length !== 0 &&
          Object.keys(crud.query).forEach(item => {
            if (crud.query[item] === null)
      ...
      //scanDateStart
      if (crud.newAddRefresh) {
        if (Array.isArray(crud.query.timeFormat)) {
          let timeList = crud.query.timeFormat;
          crud.query.scanDateStart = timeList[0];
          crud.query.scanDateEnd = timeList[1];
        }
        if (Array.isArray(crud.query.webWrongScanDateBegin)) {
          let timeList = crud.query.webWrongScanDateBegin;
          crud.query.scanDateBegin = timeList[0];
          crud.query.scanDateEnd = timeList[1];
        }
        if (Array.isArray(crud.query.webWrongDate)) {
          let timeList = crud.query.webWrongDate;
          crud.query.beginDate = timeList[0];
          crud.query.endDate = timeList[1];
        }
        if (Array.isArray(crud.query.collectionTime)) {
          let timeList = crud.query.collectionTime;
          crud.query.createBeginTime = timeList[0];
          crud.query.createEndTime = timeList[1];
        }
        if (Array.isArray(crud.query.webAccoutTime)) {
          let timeList = crud.query.webAccoutTime;
          crud.query.beginTime = timeList[0];
          crud.query.endTime = timeList[1];
        }
        if (crud.query.newAddentranceUrl) {
          crud.query.entranceUrl = crud.query.newAddentranceUrl;
        } else {
          crud.query.entranceUrl = "";
        }
        if (crud.query.nickName === '') {
          delete crud.query.nickName;
        }
      }
      return {
     ...
     * 此版本采集模块
     */
    getCollectionQueryParams: function() {
      // 清除参数无值的情况
      Object.keys(crud.query).length !== 0 &&
        Object.keys(crud.query).forEach(item => {
      ...
      if (Array.isArray(crud.query.beginDate)) {
        let timeList = crud.query.beginDate;
        crud.query.beginDate =
          timeList[0].split("-").join("/") + " " + "00:00:00";
        crud.query.endDate =
          timeList[1].split("-").join("/") + " " + "23:59:59";
      }
      return {
     ...
     */
    getDataStatus(id) {
      if (crud.dataStatus[id]) {
        return crud.dataStatus[id];
      } else {
        return { delete: 0, edit: 1 };
      }
    },
    /**
     * 用于树形表格多选, 选中所有
  ...
  Object.assign(crud, {
    ...
    // 死链/暗链
    toApplic() {
      crud.status.application = 2;
      CRUD.STATUS.APPLIC = 2;
      callVmHook(crud, CRUD.HOOK.beforeToApplic);
    },
    cancelApplic() {
      crud.status.application = 0;
      CRUD.STATUS.APPLIC = 0;
      callVmHook(crud, CRUD.HOOK.beforeToApplic);
    }
  });
...
CRUD.HOOK = {
  ...
  //死链和暗链
  beforeToApplic: "beforeCrudToApplic"
};
